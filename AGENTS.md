...
- **Speed is a feature.** The package parses dates 4–5× and intervals 4–7.5× faster than luxon on Node 24, and durations faster than luxon and iso8601-duration (see `bench/README.md` — those claims are backed by executed `npm run bench` runs and state the Node version, since the margin over luxon is wider on Node 20 and narrower on 22+; re-verify them after perf-sensitive changes). Do not introduce regex into the parser, do not allocate in the hot path (no `.split`, `.match`, `.slice` over the source, string building for numbers), and preserve the `idx`/`c`/`parsed` cursor pattern. Character classes are checked with charCode helpers (`isDigit`, `isWeekday`) or equality helpers (`isDatePrefix`, `isFraction`), digit runs accumulate numerically (`digitValue`, `twoDigits`, `POW10`), and valid-char sets are hoisted module constants — never rebuild them per call. Beware extracting helpers into the hottest loops: a shared dispatch function once regressed 15% by breaking V8 inlining. When changing perf-sensitive code, profile with `node --prof <script>` then `node --prof-process isolate-*.log` — not `console.time` micro-benchmarks.
...
- `npm run compare` — executes every capability table row in `bench/README.md` against each library and prints ✓/✗ from actual behavior, plus an error message table for malformed sources. Those tables mirror this output — keep them in sync.
...
- `npm run toc` — regenerates the table of contents between the `<!-- toc -->` markers in `README.md` and `bench/README.md` with `@0dep/toc`; `dist` runs it first, so the committed TOC always matches the headings.
...
- `.nvmrc` selects Node 24 for local work, but CI runs `npm run dist` and the tests on Node 20, 22, 24, and latest. Lint, rollup, `dts-buddy`, and `@0dep/toc` (engines `>=20`) must keep working on Node 20 — don't add dev tooling that needs a newer Node, and verify with `fnm exec --using=20 npm run dist` when touching the build chain.
...
`bench/` is a private workspace (`npm install` at root wires it, `@0dep/piso` resolves to the repo root). `bench/suites/*.js` hold tinybench throughput suites, `bench/compare.js` the capability comparison. `bench/README.md` holds the throughput and ✓/✗ tables and must reflect executed output from these scripts, not hand-estimates, together with the Node version used — re-run and update them when parser behavior or performance changes. The root README Benchmarking section only carries a one-sentence summary and a link; keep that sentence in step.
